...
int main() {
    ...
    // 行列の積
    Matrix<int> b = a * a;
    rep(i, 0, 3) { for (auto j: b[i]) {cout << j << ' ';} cout << endl;}

    // mod intを用いて、行列の100万乗を計算
    Matrix<mint> c(3);
    rep(i, 0, 3) rep(j, 0, 3) c[i][j] = (i+1) * (j+1);
    Matrix<mint> d = c ^ 1000000;
    rep(i, 0, 3) { for (auto j: d[i]) {cout << j << ' ';} cout << endl;}
}
